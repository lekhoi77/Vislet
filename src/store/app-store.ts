...
function buildTransactionRow(profileId: string, id: string, tx: Omit<Transaction, 'id' | 'createdAt'>): Record<string, unknown> {
  const row: Record<string, unknown> = {
    id, profile_id: profileId,
    type: tx.type, title: tx.title, amount: tx.amount,
    source: tx.source, category: tx.category, note: tx.note ?? '',
    date: tx.date,
    is_auto_generated: tx.isAutoGenerated ?? false,
    linked_shared_expense_id: tx.linkedSharedExpenseId ?? null,
    auto_kind: tx.autoKind ?? null,
  };
  if (tx.excludedFromReports) row.excluded_from_reports = true;
  return row;
}
...
interface AppState {
  ...
  addTransactions: (txs: Omit<Transaction, 'id' | 'createdAt'>[]) => Promise<Transaction[]>;
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  addTransaction: async (txData) => {
    ...
    const id = uuidv4();
    const row = buildTransactionRow(currentProfileId, id, txData);
  ...
  // Tạo nhiều giao dịch trong 1 lần insert (vd sau khi quét ảnh có nhiều
  // dòng giao dịch) — thay vì gọi addTransaction lặp N lần (N round-trip).
  addTransactions: async (txsData) => {
    const { currentProfileId, transactions } = get();
    if (!currentProfileId) throw new Error('Chưa có profile');
    ...
    const rows = newTxs.map(tx => buildTransactionRow(currentProfileId, tx.id, tx));
